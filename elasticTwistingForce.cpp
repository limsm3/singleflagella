...
void elasticTwistingForce::computeFt()
{
    ...
    /*for (int i=0; i<rod->ne/2-0.5; i++)
    {
        deltam(i) = -(theta_f(i) - theta_e(i));
    }

    for (int i = rod->ne/2 - 0.5; i < rod->ne - 1; i++)
    {
        deltam(i) = theta_f(i) - theta_e(i);
    }
    */
...
void elasticTwistingForce::computeJt()
{
    for (int i=1; i<rod->ne; i++)
    {
        ...
        Jtt = GJ * milen * ((deltam(i-1)+rod->refTwist(i) - rod->undeformedTwist(i)) 
