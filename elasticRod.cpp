...
elasticRod::elasticRod(MatrixXd initialNodes, MatrixXd undeformed, 
	...
	double m_youngM, double m_shearM, double m_rodLength, double m_distf)
{
...
void elasticRod::setMass()
{
	...
	for (int i=0; i<nv; i++)
	{
		...
		if (i==headNode) dm = 4/3*0.031*0.031*0.031*M_PI*rho;
		...
		if (i < nv-1)
		{
			if(i == headNode || i == headNode+1)
			{
				massArray[4 * i + 3] = rodRadius * rodRadius * (distf / 2.0) * crossSectionalArea * rho / 2.0;
			}
