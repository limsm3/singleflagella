...
void RegularizedStokeslet::prepareForViscousForce()
{
	...
	/*for (int i = 0; i < rod->nv; i++)
	{
		...
		ViscousForce(3*i) = -VelocityVec(0) * 8 * M_PI * viscosity;
		ViscousForce(3 * i+1) = -VelocityVec(1) * 8 * M_PI * viscosity;
		ViscousForce(3 * i+2) = -VelocityVec(0) * 8 * M_PI * viscosity;
		}*/
...
void RegularizedStokeslet::computeFrs()
{
	...
	for (int i = 0; i < rod->headNode-1; i++)
	{
		for (int j = 0; j < 3; j++)
		{
